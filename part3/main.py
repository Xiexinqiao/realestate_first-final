...
class Application(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.tab_control.add(self.tab3, text='Data Analysis')
    ...
    def create_tab3_widgets(self):
        ...
        analysis_button = ttk.Button(frame, text="Analyze Property Prices", command=self.analyze_property_prices)
        analysis_button.pack(pady=10)

        analysis_button = ttk.Button(frame, text="Analyze Client Budgets", command=self.analyze_client_budgets)
        analysis_button.pack(pady=10)

    def analyze_property_prices(self):
        prices = [prop.price for prop in self.properties.in_order_traversal(self.properties.root)]
        fig = Figure(figsize=(6, 4), dpi=100)
        ax = fig.add_subplot(111)
        ax.hist(prices, bins=10, color='blue', alpha=0.7)
    ...
    def analyze_client_budgets(self):
        budgets = [client.budget for client in self.clients]
        fig = Figure(figsize=(6, 4), dpi=100)
        ax = fig.add_subplot(111)
        ax.hist(budgets, bins=10, color='green', alpha=0.7)
        ax.set_title('Client Budget Distribution')
